...
CFG = {
    ...
    "s_lr_head_b":        1e-4,
    "s_phase_c_epochs":   52,      # 20 done + 32 remaining = 52 total
    "s_lr_backbone_c":    2e-6,
    "s_lr_head_c":        2e-5,

    "s_weight_decay":     1e-2,
    "student_early_stop": 15,
    ...
    # ── RESUME from ep 40 ──
    "resume_ckpt":      "studentB_EMA_C_ep40.pth",
    "resume_phase":     "C",
    "resume_global_ep": 40,
}
...
print("=" * 72)
print("  UML Student — VERSION B FIXED  (RESUMED from ep 40)")
print("=" * 72)
print(f"  Device : {CFG['device']}  |  AMP : {USE_AMP}")
print(f"  Resuming from : {CFG['resume_ckpt']}  (global ep {CFG['resume_global_ep']})")
print(f"  Running Phase C epochs 41 → 72  (32 new epochs)")
print("=" * 72)
...
# ══════════════════════════════════════════════════════════════════════
class MRIBiasField:
    def __init__(self, strength=0.15): self.s = strength
...
class CTWindowLevel:
    def __init__(self, center=40, width=80, jitter=15):
        self.center, self.width, self.jitter = center, width, jitter
    def __call__(self, img):
        a  = np.array(img, dtype=np.float32)
        ...
        return Image.fromarray(
            np.clip((a-lo)/(hi-lo)*255, 0, 255).astype(np.uint8))
...
def build_transforms(sz, modality, train=True):
    ...
    if modality == "MRI":
        ...
        return transforms.Compose(base + [
            ...
            CTWindowLevel(CFG["ct_window_center"],CFG["ct_window_width"],
...
class UMLStudentB(nn.Module):
    ...
    def forward(self, x, modality):
        ...
        if modality == "MRI":
            proj = self.mri_proj(feat); logits = self.mri_head(proj)
        else:
            proj = self.ct_proj(feat);  logits = self.ct_head(proj)
        return logits, proj
    ...
    def head_params(self):
        return (list(self.mri_proj.parameters())
...
def uncertainty_weighted_kd(s_logits, t_logits, la, lb, lam, cfg):
    ...
    ce = mixup_ce(s_logits, la, lb, lam, cfg["label_smoothing"])
...
def save_plots(history, phase_tag):
    ...
    axes[2].set_yscale("log"); axes[2].set_xlabel("Epoch")
...
def run_phase(phase, n_epochs, lr_bb, lr_hd, ep_offset,
              ...
              history, cfg, scaler):
    dev          = cfg["device"]
    steps_per_ep = (len(mri_train) + len(ct_train)) // 2
    best_avg     = 0.0
    ...
    if lr_bb is None:
        ...
        sched = make_scheduler(opt, [lr_bb,lr_hd],
                               steps_per_ep*2, n_epochs, cfg)

    ep_label = (f"backbone FROZEN" if lr_bb is None
                else f"bb_lr={lr_bb:.0e}")
    print(f"\n  ── Phase-S-{phase} ({ep_label})  "
          f"[{n_epochs} epochs, global ep "
          f"{ep_offset+1}–{ep_offset+n_epochs}] ──")
    ...
    for epoch in range(n_epochs):
        ...
        for step in range(steps_per_ep):
            ...
            with autocast(enabled=USE_AMP):
                ...
                if phase=="C" and last_ct_proj is not None and ct_guard_ok:
                    idx = torch.randperm(
                        last_ct_proj.size(0), device=dev)[:s_mri_proj.size(0)]
            ...
            if USE_AMP:
                ...
                torch.nn.utils.clip_grad_norm_(
                    student.parameters(), cfg["grad_clip"])
                opt.step()
            sched.step(); ema.update(student)
            mb_loss.append(loss_m.item())

            # ── CT step ───────────────────────────────────────────
            try:    im_c, lb_c = next(ct_iter)
            except:
                ct_iter = iter(ct_train)
            ...
            with autocast(enabled=USE_AMP):
                ...
                if phase=="C" and ct_guard_ok:
                    idx = torch.randperm(
                        s_mri_proj.size(0), device=dev)[:s_ct_proj.size(0)]
        ...
        print(f"  [{phase}] Ep {glep:3d}  lr={cur_lr:.2e}  "
              f"MRI[loss={np.mean(mb_loss):.4f}|val={acc_m:.4f}"
              f"|H={phm} T={ptm}]  "
              f"CT[loss={np.mean(cb_loss):.4f}|val={acc_c:.4f}"
              f"|H={phc} T={ptc}]  "
              f"avg={avg:.4f}  {guard_str}")

        if glep % cfg["student_ckpt_every"] == 0:
            save_ckpt(ema_model.state_dict(),
                      f"studentB_EMA_{phase}_ep{glep}.pth")
...
# FINAL EVALUATION
# ══════════════════════════════════════════════════════════════════════
def full_evaluation(student, mri_teacher, ct_teacher,
                    mri_val, ct_val, cfg):
    dev = cfg["device"]
    ...
    summary = {}
    lines   = ["VERSION B FIXED — RESUMED FROM EP 40", "="*72]
    ...
    for tag, mdl, ldr, mod in configs:
        ...
        cr = classification_report(
            m["labels"], m["preds"],
            target_names=["Healthy","Tumor"], zero_division=0)
        blk = (f"\n  ── {tag} ──\n"
               f"    Accuracy  : {m['acc']:.4f}\n"
               f"    AUC-ROC   : {m['auc']:.4f}\n"
    ...
    hdr = (f"\n  {'Model':<{w}} {'Acc':>7} {'AUC':>7} "
           f"{'Prec':>7} {'Rec':>7} {'F1':>7}")
    div = "  " + "─"*(w+38)
    print(hdr); print(div); lines += [hdr, div]
    ...
    log_path = os.path.join(LOG_DIR, "final_results_vB_resumed_ep40.txt")
    ...
    plt.tight_layout()
    cm_path = os.path.join(PLOT_DIR, "confusion_matrices_resumed_ep40.png")
...
mri_teacher.load_state_dict(torch.load(
    os.path.join(CFG["teacher_ckpt_dir"], "mri_teacher_BEST.pth"),
    map_location=CFG["device"]))
ct_teacher.load_state_dict(torch.load(
    os.path.join(CFG["teacher_ckpt_dir"], "ct_teacher_BEST.pth"),
    map_location=CFG["device"]))
...
# ── Load student from ep 40 checkpoint ────────────────────────────
print(f"\n  Loading student from {CFG['resume_ckpt']} ...")
student   = UMLStudentB(CFG["num_classes"]).to(CFG["device"])
ckpt_path = os.path.join(CKPT_DIR, CFG["resume_ckpt"])
...
# ── Phase C resumed: ep 41 → 72 (32 epochs) ───────────────────────
phase_b_end = CFG["s_phase_a_epochs"] + CFG["s_phase_b_epochs"]  # 20
c_done      = CFG["resume_global_ep"] - phase_b_end               # 20
c_left      = CFG["s_phase_c_epochs"] - c_done                    # 32
c_offset    = phase_b_end + c_done                                 # 40

print(f"\n{'━'*72}")
print(f"  PHASE C (RESUMED) — {c_left} epochs  "
      f"(global ep {c_offset+1}–{c_offset+c_left})")
print(f"{'━'*72}")
...
save_plots(history, "phase_C_resumed_ep40")
print(f"\n  Phase C (resumed) complete  best avg = {best_avg_c:.4f}")

full_evaluation(student, mri_teacher, ct_teacher,
                loaders["MRI_val"], loaders["CT_val"], CFG)

elapsed = time.time() - t0
print(f"\n{'='*72}")
print(f"  ✅  RESUMED TRAINING COMPLETE  ({elapsed/60:.1f} min)")
print(f"  Best ckpt : {CKPT_DIR}/studentB_C_RESUMED_BEST.pth")
print(f"{'='*72}")
